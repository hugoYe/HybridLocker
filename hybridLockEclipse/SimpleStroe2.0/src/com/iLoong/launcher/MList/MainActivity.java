...
public class MainActivity extends Activity
{
			...
			Bundle savedInstanceState )
	{
		...
		{
			mainJsClass = new JSClass( mainWebView , "M" , 0 , getId() , mHandler , R , MeApkDLShowType.WebviewMain );
		}
		else
		{
			mainJsClass = new JSClass( mainWebView , "M" , getId() , getId() , mHandler , R , MeApkDLShowType.WebviewMain );
		}
		MeApkDownloadManager.MeAddActiveCallBackMap.put( MeApkDLShowType.WebviewMain , new MeActiveCallback() {
		...
		{
			subJsCalss = new JSClass( subWebView , "M" , getId() , getId() , mHandler , R , MeApkDLShowType.WebviewSub );
		}
		MeApkDownloadManager.MeAddActiveCallBackMap.put( MeApkDLShowType.WebviewSub , new MeActiveCallback() {
		...
		MELOG.v( "ME_RTFSC" , "id:" + getId() + " strAction:" + strAction + ", strActionDescription:" + strActionDescription );
		...
		{
			MELOG.v( "ME_RTFSC" , "detailbox_pkgname  detailbox_pkgname  detailbox_pkgname " );
			...
			subWebView.loadUrl( " http://uifolder.coolauncher.com.cn/qqkhd/detailbox.htm?pkgname=" + strActionDescription );
			LoadMiroEntryUrl( null , false );
		}
		else if( strAction.equals( "detailfolder_pkgname" ) )
		{
			MELOG.v( "ME_RTFSC" , "detailfolder_pkgname  detailfolder_pkgname  detailfolder_pkgname " );
			SubFrameWebview.setVisibility( View.VISIBLE );
			MainFrameWebview.setVisibility( View.INVISIBLE );
			subWebView.clearView();
			subJsCalss.setDialog();
			isNeedClearHistory = true;
			subWebView.loadUrl( "http://uifolder.coolauncher.com.cn/qqkhd/detail.php?p=" + LoadURL.BaseDetalFolder64Str( this , strActionDescription ) );
			LoadMiroEntryUrl( null , false );
		}
		else if( strAction.equals( "pkgname" ) )
		{
			...
			subJsCalss.setDialog();
			isNeedClearHistory = true;
			subWebView.loadUrl( " http://uifolder.coolauncher.com.cn/qqkhd/detailpush.htm?pkgname=" + strActionDescription );
			LoadMiroEntryUrl( null , false );
		}
			...
			boolean isNeedShowPregressDlg )
	{
		...
		{
			url = url + "?" + "p=" + LoadURL.Base64Str( MainActivity.this , getId() );
			//MELOG.v( "ME_RTFSC" , "1111  mainWebView.loadUrl" + url );
		}
		else
		{
			RijndaelCrypt aes = new RijndaelCrypt( RijndaelCrypt.PWD , RijndaelCrypt.IV );
			url = aes.decrypt( UPath ) + "?p=" + LoadURL.Base64Str( MainActivity.this , getId() );
...
//		finish();
			...
			WebView webView )
	{
		...
		else if( true == JSClass.IsNetworkAvailableLocal( getApplicationContext() ) )
		{
		...
		webView.setWebViewClient( new WebViewClient() {
			...
			{// 网页加载完毕
				...
				if( flagError == true && "file:///android_asset/cool_ml_NoNet.htm".equals( url ) )
				{
					//view.loadUrl( "javascript:document.body.innerHTML= ' ' " );
					//view.loadUrl( "javascript:window.location.replace('file:///android_asset/cool_ml_NoNet.htm');" );
					//view.goBack();
