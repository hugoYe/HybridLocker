...
public class JSClass
{
			...
			MeApkDLShowType curShowType )
	{
		...
		MeApkDlMgr = MeApkDlMgrBuilder.Build( context.getApplicationContext() , moudleName , mDLManagerID );
	...
	{
		MainActivityHandler.sendEmptyMessage( MainActivity.subWebViewBackSoftKey );
	}
	...
	{
		//MELOG.v( "ME_RTFSC" , "=============== DownloadApkNeedDownload =================== " );
		if( !MeGeneralMethod.IsWifiConnected( context ) )
		{
			return;
		}
		//MELOG.v( "ME_RTFSC" , " DownloadApkNeedDownload 111111 " );
		List<dl_info> DlInfoList = MeApkDlMgr.GetSdkApkDlMgr().ResGetTaskListNeedDownload();
		...
		{
			MELOG.v( "ME_RTFSC" , "DownloadApkNeedDownload: pkgNameLsit:" + pkgNameLsit );
			Intent mIntent = new Intent( context , MEServiceActivity.class );
			mIntent.putExtra( "MeServiceType" , MeServiceType.MEApkReStartByEntryID );
		...
		webView.post( new Runnable() {
			...
			public void run()
			{
				...
				MeApkDlMgr.GetSdkIconMgr().IconDownload( pkgName , new CoolDLCallback() {
					...
					{
						// TODO Auto-generated method stub
						Intent intent = new Intent( "com.cooee.microEntrce.download" );
					...
					dl_cb dl = new dl_cb() {
								...
								dl_info info ) throws Exception
						{
							...
							if( !IsForegroundRunning( context ) )
							{
								if( MicroEntryHelper.shouldExit( context ) )
								{
									//MeGeneralMethod.stopMeDLProtectionService( context.getApplicationContext() );
									//android.os.Process.killProcess( android.os.Process.myPid() );
								}
							}
							//AppInstall( pkgName );
				...
				if( null == MeApkDlMgr.GetSdkApkDlMgr().ResGetInfo( CoolDLResType.RES_TYPE_APK , pkgName ) )
				{
					try
					{
						...
						MeApkDlMgr.StartDownload( curShowType , pkgName , appName , size , versionName , mEntryId , WebMainApkDownloadCallBack );
					}
					catch( Exception e )
					{
						// TODO Auto-generated catch block
						MELOG.e( "ME_RTFSC" , "DownloadApk:onDoing Error:" + e.toString() );
						e.printStackTrace();
					}
				}
	...
	@JavascriptInterface
	public void LoadUrlByBrowserByGP(
			String url )
	{
		try
		{
	...
	@JavascriptInterface
	public boolean startAppStoreDownload(
			String pkgName )
	{
		try
		{
			context.startActivity( new Intent( Intent.ACTION_VIEW ).setData( Uri.parse( "market://details?id=" + pkgName ) ) );
			MELOG.v( "ME_RTFSC" , " ====== appstore start===== " );
			return true;
		}
		catch( Exception e2 )
		{
			MELOG.v( "ME_RTFSC" , " ====== start appstore Activity error:" + e2.getMessage() );
	...
	/*
	private boolean isGoogleLink(
			String url )
	{
		return ( url.indexOf( "https://play.google" ) >= 0 ) || ( url.indexOf( "http://play.google" ) >= 0 );
	}
	*/
