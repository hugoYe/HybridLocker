...
public class MyReceiver extends BroadcastReceiver
{
			...
			Context context )
	{
		...
		for( ActivityManager.RunningAppProcessInfo amService : mRunningService )
		{
			...
			MELOG.v( "ME_RTFSC" , "mypid:" + mypid  +  "  amService.pid:" + amService.pid  + "  amService.importance:" + amService.importance);
			...
			Intent intent )
	{
		...
		mContent = context;
		MELOG.v( "ME_RTFSC" , intent.getAction() + "====MyReceiver onReceive =====  " );
		...
		if( intent.getAction().equals( Intent.ACTION_PACKAGE_ADDED ) || intent.getAction().equals( Intent.ACTION_PACKAGE_REPLACED ) )
		{
			...
			new Thread( new Runnable() {
				...
				public void run()
				{
					...
					for( int i = 0 ; i <= 4 ; i++ )
					{
						int EntryId = microEntryHelper.getInt( pkgName + MeServiceType.MEApkOnSucess + i , -1 );
						if( -1 != EntryId )
						{
							PkgAddedEntryIDList.add( EntryId );
							microEntryHelper.setValue( pkgName + MeServiceType.MEApkOnSucess + i , -1 );
						}
					}
					MELOG.v( "ME_RTFSC" , "PkgAddedEntryIDList:" + PkgAddedEntryIDList );
					if( !PkgAddedEntryIDList.isEmpty() || IsMeForeground( mContent ) )
					{
		...
		if( intent.getAction().equals( Intent.ACTION_PACKAGE_REMOVED ) )
		{
			...
			new Thread( new Runnable() {
				...
				public void run()
				{
					// TODO Auto-generated method stub
					MicroEntryHelper microEntryHelper = MicroEntryHelper.getInstance( mContent );
					if( "TRUE".equals( microEntryHelper.getString( pkgName + MeServiceType.MeApkOnInstalled ) ) || IsMeForeground( mContent ) )
					{
		...
		if( intent.getAction().equals( "android.net.conn.CONNECTIVITY_CHANGE" ) )
		{
			new Thread( new Runnable() {
				...
				public void run()
				{
					...
					for( int i = 0 ; i <= 4 ; i++ )
					{
						CoolDLMgr dlmgr = MicroEntry.CoolDLMgr( mContent , "M" , i );
						List<dl_info> DlInfoList = dlmgr.ResGetTaskListNeedDownload();
					...
					if(null != entryIDLsit && null != Listmap && !entryIDLsit.isEmpty()&&!Listmap.isEmpty() )
					{
						Intent mIntent = new Intent( mContent , MEServiceActivity.class );
						mIntent.setFlags( Intent.FLAG_ACTIVITY_NEW_TASK );
						mIntent.putExtra( "MeServiceType" , MeServiceType.MEApkReStartAll );
						mIntent.putExtra( "moudleName" , "M" );
						mIntent.putIntegerArrayListExtra( "entryIDList" , entryIDLsit );
						for( int entryID : entryIDLsit )
						{
							mIntent.putStringArrayListExtra( "PkgNameList"+entryID , Listmap.get( entryID ) );
						}
