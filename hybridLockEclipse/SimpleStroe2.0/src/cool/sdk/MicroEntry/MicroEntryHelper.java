...
public class MicroEntryHelper extends MicroEntryUpdate
{
	...
	public void OnDataChange() throws Exception
	{
		...
		else
		{
			...
			for( int i = 0 ; i < 4 ; i++ )
			{
				setEnabled( context , mActivityClass[i] , visible[i] , visableInDesktop[i] , i );
			}
		}
	}
			...
			if( true == visible[i] )
			{
				...
				if( true == checkActionCanEnable( context , i ) )
				{
					...
					else
					{
						...
						MELOG.v( "ME_RTFSC" , "NotifyCount:" + NotifyCount + " c0:" + c0 );
						//钝化后强制显示
						...
						else
						{
							...
							{
								long PerTime = (long)getLong( "ME_SHOW_SERVE_PERTIME" , 0L );
								long TimeDiff = CurTime - PerTime;
								MELOG.v( "ME_RTFSC" , "PerTime:" + PerTime + "   TimeDiff:" + TimeDiff );
								if( TimeDiff > TimeWakeUpShow )
								{
									if( true == HasSetDisable )
									{
										ShowNotifcationPer( visible , i , CurTime , NotifyCount , c0 );
									}
									else
									{
										ShowMENotification( visible , i , CurTime , NotifyCount , c0 );
									}
									break;
								}
								else if( TimeDiff > TimeShow && 5 != NotifyCount )
								{
									if( true == HasSetDisable )
									{
										ShowNotifcationPer( visible , i , CurTime , NotifyCount , c0 );
	...
	private void ShowMENotification(
			boolean[] visible ,
			int MEindex ,
			long CurTime ,
			int NotifyCount ,
			int NoitfType )
	{
		...
		Notification notification = new Notification( RR.drawable.cool_ml_know , context.getString( RR.string.cool_ml_new_content ) , System.currentTimeMillis() );
		Intent notificationIntent = new Intent( context , com.iLoong.launcher.MList.MEServiceActivity.class );
		...
		notification.setLatestEventInfo( context , context.getString( RR.string.cool_ml_new_content ) , context.getString( RR.string.cool_ml_more_content ) , contentItent );
		...
		for( int i = 0 ; i < visableInDesktop.length ; i++ )
		{
			...
			MELOG.v( "ME_RTFSC" , "ShowMENotification ShortcurtVisable " + i + " " + visableInDesktop[i] );
		}
	}
	
	private void ShowNotifcationPer(
			boolean[] visible ,
			int MEindex ,
			long CurTime ,
			int NotifyCount ,
			int NoitfType )
	{
		...
		for( int i = 0 ; i < visableInDesktop.length ; i++ )
		{
			...
			MELOG.v( "ME_RTFSC" , "ShowNotifcationPer ShortcurtVisable " + i + " " + visableInDesktop[i] );
		}
	...
	public void ShowNotifcation()
	{
		...
		if( -1 != MEindex )
		{
			...
			ShowMENotification( visible , MEindex , CurTime , NotifyCount , NoitfType );
		}
	...
	public void UpdateMeStateUserConfirm(
			boolean visible[] ,
			int index )
	{
		...
		for( int i = 0 ; i < ShortcurtVisable.length ; i++ )
		{
			...
			MELOG.v( "ME_RTFSC" , "ShortcurtVisable " + i + " " + ShortcurtVisable[i] );
		}
		for( int i = 0 ; i < 4 ; i++ )
		{
			...
			if( true == visible[i] )
			{
				setEnabled( context , mActivityClass[i] , visible[i] , ShortcurtVisable[i] , i );
			}
		}
	}
	
	private void updateMEStateForce(
			boolean visible[] )
	{
		// TODO Auto-generated method stub
	...
	public void SetDesktopShotCutIcon(
			Context context ,
			Class<?> mClass ,
			boolean isEnable ,
			int i )
	{
		//MELOG.v( "ME_RTFSC" , "SetDesktopShotCutIcon:" + mClass + ":" + isEnable );
		int[] shortCutNameID = { RR.string.cool_ml_app_name1 , RR.string.cool_ml_app_name2 , RR.string.cool_ml_app_name3 , RR.string.cool_ml_app_name4 };
		int[] shortCutIconID = { RR.drawable.cool_ml_wonderful_game , RR.drawable.cool_ml_software , RR.drawable.cool_ml_ku_store , RR.drawable.cool_ml_know };
		if( true == isEnable )
		{
			...
			shortcutIntent.putExtra( Intent.EXTRA_SHORTCUT_NAME , context.getString( shortCutNameID[i] ) );
			shortcutIntent.putExtra( Intent.EXTRA_SHORTCUT_ICON_RESOURCE , Intent.ShortcutIconResource.fromContext( context , shortCutIconID[i] ) );
			MELOG.v( "ME_RTFSC" , "INSTALL_SHORTCUT: shortCutName:" + context.getString( shortCutNameID[i] ) + "shortCutIconID:" + shortCutIconID[i] );
		...
		else if( false == isEnable )
		{
			...
			shortcutIntent.putExtra( Intent.EXTRA_SHORTCUT_NAME , context.getString( shortCutNameID[i] ) );
			//shortcutIntent.putExtra( Intent.EXTRA_SHORTCUT_ICON_RESOURCE , Intent.ShortcutIconResource.fromContext( context , shortCutIconID[i] ) );
			MELOG.v( "ME_RTFSC" , "UNINSTALL_SHORTCUT: shortCutName:" + context.getString( shortCutNameID[i] ) + "shortCutIconID:" + shortCutIconID[i] );
			context.sendBroadcast( shortcutIntent );
			//MeLauncherInterface.getInstance().UninstallShortcut( shortcutIntent );
		}
	}
	...
	public void setEnabled(
			...
			Class<?> mClass ,
			boolean isenable ,
			boolean isDesktopEnable ,
			int i )
	{
		MELOG.v( "ME_RTFSC" , "setEnabled: " + isenable + isDesktopEnable );
		if( !isenable && true == checkActionCanDisable( context , i ) && 1 != getInt( mClass.toString() , 0 ) )
		{
			setComponentEnabled( context , new ComponentName( context , mClass ) , PackageManager.COMPONENT_ENABLED_STATE_DISABLED );
			if( !context.getApplicationContext().getPackageName().equals( "com.cooee.oilauncher" ) && !context.getApplicationContext().getPackageName().equals( "com.cooee.unilauncher" ) )
			{
		...
		else if( isenable && true == checkActionCanEnable( context , i ) )
		{
			...
			if( true == isDesktopEnable && !context.getApplicationContext().getPackageName().equals( "com.cooee.oilauncher" ) && !context.getApplicationContext().getPackageName()
	...
	public void setEnableByCfg(
			int i )
	{
		if( true == checkActionCanEnable( mContext , i ) )
		{
			setComponentEnabled( mContext , new ComponentName( mContext , mActivityClass[i] ) , PackageManager.COMPONENT_ENABLED_STATE_ENABLED );
		}
	}
	
	private boolean checkActionCanEnable(
			Context context ,
			int i )
	{
		...
		try
		{
			...
			listActivity = context.getPackageManager().queryIntentActivities( in , 0 );
		...
		if( true == hasEnable )
		{
			...
			ComponentName componentName = new ComponentName( context.getApplicationContext().getPackageName() , mActivityClass[i].getName() );
			if( true == PreferenceManager.getDefaultSharedPreferences( context.getApplicationContext() ).getBoolean( "HIDE:" + componentName.toString() , false ) )
			{
				hasEnable = false;
			}
			//判断是否被手动卸载了（原生态 Launcher）
			if( true == PreferenceManager.getDefaultSharedPreferences( context.getApplicationContext() ).getBoolean( "ME_HIDE:" + mActivityClass[i].getName() , false ) )
			{
				hasEnable = false;
			}
		}
		
		MELOG.v( "ME_RTFSC" , "checkActionCanEnable:" + LoadURL.hexStr2Str( MEAction[i] ) + "====" + hasEnable );
		return hasEnable;
	}
	
	private boolean checkActionCanDisable(
			Context context ,
			int i )
	{
		Intent in = new Intent();
		...
		if(true == hasDisable)
		{
			if( true == PreferenceManager.getDefaultSharedPreferences( context.getApplicationContext() ).getBoolean( "meconfig:" + mActivityClass[i].getName() , false ) )
			{
				hasDisable = false;
			}
		}
		
		MELOG.v( "ME_RTFSC" , "checkActionDisable:" + LoadURL.hexStr2Str( MEAction[i] ) + "====" + hasDisable );
		return hasDisable;
	}
			...
			int newStat )
	{
		try
		{
			...
			MELOG.v( "ME_RTFSC" , compName.toShortString() + ":" + newStat + pkgMgr.getComponentEnabledSetting( compName ) );
			if( pkgMgr.getComponentEnabledSetting( compName ) != newStat )
			{
				...
				if( PackageManager.COMPONENT_ENABLED_STATE_DISABLED == newStat && false == HasSetDisable )
				{
					HasSetDisable = true;
				}
			}
	...
	public void UpdateDeleteItemByHide(
			int type ,
			String className )
	{
		MELOG.v( "ME_RTFSC" , "===UpdateDeleteItemListByHide===  className:" + className + "  type:" + type );
		String[] name = {
	...
	private void UpdateDeleteItemListByConfig(
			boolean[] visible )
	{
		MELOG.v( "ME_RTFSC" , "=== UpdateDeleteItemListByConfig ===" );
		List<MicroEntryLogItem> DeleteItemList = null;
		int[] MENameID = { RR.string.cool_ml_app_name1 , RR.string.cool_ml_app_name2 , RR.string.cool_ml_app_name3 , RR.string.cool_ml_app_name4 };
		DeleteItemList = new ArrayList<MicroEntryLog.MicroEntryLogItem>();
		for( int i = 0 ; i < 4 ; i++ )
		{
			if( false == visible[i] && true == checkActionCanDisable( context , i ) )
			{
				MicroEntryLogItem myItem = new MicroEntryLogItem();
				myItem.id = i + 1;
				myItem.type = 3;
	...
	private void UpdateActiveItemList(
			boolean[] visible ,
			int type )
	{
