...
public class MessageNotify
{
			...
			Context context )
	{
		...
		try
		{
			csr = context.getContentResolver().query( smsUri , null , "type = 1 and read = 0" , null , null );
			if( csr != null )
			{
			...
			Context context )
	{
		...
		try
		{
			csr = context.getContentResolver().query( mmsUri , null , "read = 0" , null , null );
			if( csr != null )
			{
				smsCount = csr.getCount();
			}
	...
	private void registerMMSObserver(
			...
			ContentObserver observer )
	{
		context.getContentResolver().registerContentObserver( mmsUri , true , observer );
	}
