...
public class LockView extends View implements IBaseView
{
	...
	private BroadcastReceiver mChangeReceiver = new BroadcastReceiver() {
				...
				Intent intent )
		{
			...
			if( action.equals( Intent.ACTION_BATTERY_CHANGED ) )
			{
				...
				int statePower = intent.getIntExtra( "status" , BatteryManager.BATTERY_STATUS_UNKNOWN );
				if( statePower == BatteryManager.BATTERY_STATUS_CHARGING || statePower == BatteryManager.BATTERY_STATUS_FULL )
				{
					mIsCharge = true;
					if( mPm.isScreenOn() )
					{
	...
	private void initDrawable()
	{
		...
		if( mDrawableBg != null )
		{
			float sc = Math.max( (float)mScreenWidth / (float)mDrawableBg.getIntrinsicWidth() , (float)mScreenHeight / (float)mDrawableBg.getIntrinsicHeight() );
			mDrawableBg.setBounds(
					...
					(int)( mScreenWidth - mDrawableBg.getIntrinsicWidth() * sc ) / 2 + (int)( mDrawableBg.getIntrinsicWidth() * sc ) ,
					(int)( mScreenHeight - mDrawableBg.getIntrinsicHeight() * sc ) / 2 + (int)( mDrawableBg.getIntrinsicHeight() * sc ) );
		}
	...
	private void StartStatisticsService()
	{
		if( mUseStatistics )
		{
			...
			try
			{
				launcherVersion = mPackageManager.getPackageInfo( mContext.getPackageName() , 0 ).versionCode;
			}
			...
			Canvas canvas )
	{
		if( mIsCharge )
		{
			...
			canvas.drawText( string , ( mScreenWidth - mPaint.measureText( string ) ) / 2 , ( TIME_Y + mDrawableTime[0].getIntrinsicHeight() + 122 ) * mScaleX , mPaint );
		}
	}
			...
			Canvas canvas )
	{
		int sc = canvas.saveLayer(
				...
				(int)( ( mScreenWidth - mDrawableLephone.getIntrinsicWidth() * mScaleX ) / 2 + mDrawableLephone.getIntrinsicWidth() * mScaleX ) ,
				(int)( mScreenHeight - LEPHONE_Y * mScaleX + mDrawableLephone.getIntrinsicHeight() * mScaleX ) ,
				null ,
		...
		mDrawableMask.setBounds(
				(int)( ( mScreenWidth - mDrawableLephone.getIntrinsicWidth() * mScaleX ) / 2 + mDrawableLephone.getIntrinsicWidth() * mScaleX - mDrawableMask.getIntrinsicWidth() * mScaleX + mMoveX ) ,
				(int)( mScreenHeight - LEPHONE_Y * mScaleX ) ,
				(int)( ( mScreenWidth - mDrawableLephone.getIntrinsicWidth() * mScaleX ) / 2 + mDrawableLephone.getIntrinsicWidth() * mScaleX + mMoveX ) ,
				(int)( mScreenHeight - LEPHONE_Y * mScaleX + mDrawableLephone.getIntrinsicHeight() * mScaleX ) );
		mDrawableMask.draw( canvas );
		( (BitmapDrawable)mDrawableLephone ).getPaint().setXfermode( new PorterDuffXfermode( Mode.MULTIPLY ) );
			...
			Canvas canvas )
	{
		...
		mDrawableUnlock.setBounds(
				...
				(int)( ( mScreenWidth - mDrawableUnlock.getIntrinsicWidth() * mScaleX ) / 2 + mDrawableUnlock.getIntrinsicWidth() * mScaleX ) ,
				(int)( mScreenHeight - UNLOCK_Y * mScaleX + mDrawableUnlock.getIntrinsicHeight() * mScaleX ) );
			...
			Canvas canvas )
	{
		if( mHour / 10 == 0 )
		{
			mTimeWidth = ( 3 * ( mDrawableTime[0].getIntrinsicWidth() + TIME_GAP ) + mDrawableColon.getIntrinsicWidth() ) * mScaleX;
		}
		else
		{
			mTimeWidth = ( 4 * ( mDrawableTime[0].getIntrinsicWidth() + TIME_GAP ) + mDrawableColon.getIntrinsicWidth() ) * mScaleX;
		}
		...
		mDrawableTime[mMinute % 10].setBounds(
				...
				(int)( TIME_Y * mScaleX + mDrawableTime[0].getIntrinsicHeight() * mScaleX ) );
		mDrawableTime[mMinute % 10].draw( canvas );
		mWeekX = TIME_X + mDrawableTime[0].getIntrinsicWidth() * mScaleX - mPaint.measureText( mStringWeek );
		...
		canvas.drawText( mStringDate , mDateX , ( TIME_Y + mDrawableTime[0].getIntrinsicHeight() + 44 ) * mScaleX , mPaint );
		canvas.drawText( mStringWeek , mWeekX , ( TIME_Y + mDrawableTime[0].getIntrinsicHeight() + 44 ) * mScaleX , mPaint );
	}
